...
var Post = {
    ...
    getIdsOfType: function ({ orderby = "post_time", direction = "desc", type = "story" }, callback) {
        console.log("getIdsOfType", type)
        if (type === "story" || type === "comment") {
            return db.query("select id from post where post_type=? order by ? ?", [type, orderby, direction], callback);
        } 
    },
    ...
    updatePost: function (id, Post, callback) {
        ...
        return db.query("update post set post_title=?,post_text=?,post_url=? where id=? AND username=? AND pwd_hash=?", [
            Post.post_title,
            Post.post_text,
            Post.post_url,
            id,
    ...
    updateVote: function ({
        ...
        console.log("insertVote", username, id, value)
        return db.query("update vote set (username,post_id,value,id) values (?,?,?,default)", [username, id, value], callback);
    },
